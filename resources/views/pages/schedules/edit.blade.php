...
@section('page-header')
  ...
  <div class="breadcrumb-header justify-content-between">
    <div class="left-content">
      <div>
        <h2 class="main-content-title tx-24 mg-b-1 mg-b-lg-1"> ✏️ تعديل الجدول الدراسي </h2>
        <p class="mg-b-0"> تعديل معلومات الجدول الدراسي </p>
      </div>
    </div>
...
@section('content')
  <div class="container-fluid">
      ...
      <div class="row align-items-center">
        <div class="col-md-8">
          <div class="d-flex align-items-center">
            ...
            <div>
              <h1 class="mb-2 fw-bold">تعديل الجدول الدراسي</h1>
              <p class="mb-0 opacity-75">قم بتعديل معلومات الجدول الدراسي</p>
            </div>
          </div>
        </div>
        <div class="col-md-4">
          <div class="d-flex justify-content-end gap-2">
            <a href="{{ route('schedules.show', $schedule->id) }}" class="btn btn-light btn-custom">
    ...
    <div class="edit-form-container">
      ...
      <form action="{{ route('schedules.update', $schedule->id) }}" method="POST" enctype="multipart/form-data" id="editForm" class="needs-validation" novalidate>
        @csrf
        @method('PUT')
          ...
          <div class="row g-3">
            <div class="col-md-4">
              ...
              <select name="type" id="type" class="form-select" required>
                ...
                <option value="جداول الامتحانات" {{ old('type', $schedule->type) == 'جداول الامتحانات' ? 'selected' : '' }}>جداول الامتحانات</option>
                <option value="جداول الدرسه" {{ old('type', $schedule->type) == 'جداول الدرسه' ? 'selected' : '' }}>جداول الدرسه</option>
                <option value="غير ذالك" {{ old('type', $schedule->type) == 'غير ذالك' ? 'selected' : '' }}>غير ذالك</option>
              </select>
            ...
            <div class="col-md-4">
              ...
              <select name="class" id="class" class="form-select" required>
                ...
                <option value="الترم الاول" {{ old('class', $schedule->class) == 'الترم الاول' ? 'selected' : '' }}>الترم الاول</option>
                <option value="الترم الثاني" {{ old('class', $schedule->class) == 'الترم الثاني' ? 'selected' : '' }}>الترم الثاني</option>
              </select>
            ...
            <div class="col-md-4">
              ...
              <input type="date" class="form-control" id="academic_year" name="academic_year" value="{{ old('academic_year', $schedule->academic_year) }}"
                required>
            ...
            <div class="col-md-6">
              ...
              <select name="department_id" id="department_id" class="form-select" required>
                ...
                @foreach ($departments as $department)
                  <option value="{{ $department->id }}" {{ old('department_id', $schedule->department_id) == $department->id ? 'selected' : '' }}>
                    {{ $department->name }}
                  </option>
                @endforeach
              </select>
            ...
            <div class="col-md-6">
              ...
              <select name="level_id" id="level_id" class="form-select" required>
                ...
                @foreach ($levelAcademics as $level)
                  <option value="{{ $level->id }}" {{ old('level_id', $schedule->level_id) == $level->id ? 'selected' : '' }}>
                    {{ $level->level_name }}
                  </option>
                @endforeach
              </select>
        ...
        <div class="form-section">
          ...
          @if($schedule->file_path)
            <div class="current-file">
              <h6 class="mb-2">
                <i class="bi bi-file-earmark-pdf me-2"></i>الملف الحالي
              </h6>
              <div class="file-info">
                <div>
                  <p class="mb-1">ملف PDF للجدول الدراسي</p>
                  <small class="opacity-75">تم رفع الملف في: {{ $schedule->created_at->format('Y-m-d H:i') }}</small>
                </div>
          ...
          <div class="file-upload-area" id="file-upload-area">
            ...
            <input type="file" class="form-control" id="file_path" name="file_path" accept=".pdf,.jpg,.jpeg,.png" style="display: none;">
        ...
        <div class="row">
          <div class="col-12">
            <div class="d-flex justify-content-center gap-3 mt-4">
              ...
              <a href="{{ route('schedules.show', $schedule->id) }}" class="btn btn-info btn-custom btn-lg px-5">
                <i class="bi bi-eye me-2"></i>معاينة
              </a>
              <a href="{{ route('schedules.index') }}" class="btn btn-secondary btn-custom btn-lg px-5">
