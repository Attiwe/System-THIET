    ...
    <div class="modal-content shadow-lg border-0 rounded-3">
      ...
      <div class="modal-header text-white">
        <h5 class="modal-title fw-bold" id="staticBackdropLabel">
          <i class="bi bi-diagram-3"></i> تعديل ملف مجلس الإدارة
        </h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <div class="modal-body">
        <form action="{{ route('management-boards.update', $item->id) }}" method="POST" enctype="multipart/form-data">
          @csrf
          @method('PUT')  
          ...
          <div class="mb-3">
            ...
            @if($item->file_path)
              <small class="text-muted">ملف حالي: <a href="{{ Storage::url($item->file_path) }}" target="_blank">عرض</a></small>
            @endif
          </div>
