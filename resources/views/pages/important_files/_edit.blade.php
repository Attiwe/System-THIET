    ...
    <div class="modal-content shadow-lg border-0 rounded-3">
      ...
      <div class="modal-header text-white">
        <h5 class="modal-title fw-bold" id="staticBackdropLabel">
          <i class="bi bi-folder-check"></i> تعديل ملف هام
        </h5>
        <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>

      <div class="modal-body">
        <form action="{{ route('important-files.update', $item->id) }}" method="POST" enctype="multipart/form-data">
          @csrf
          @method('PUT')  
          ...
          <div class="row g-3">
            ...
            <div class="col-md-6">
              ...
              <input type="text" class="form-control form-control-lg" id="title" name="title" placeholder="مثال: لائحة السلوك الجامعي" value="{{ $item->title }}" required>
            </div>
            ...
            @if($item->file_path)
              <div class="col-12">
                <small class="text-muted">ملف حالي: <a href="{{ route('important-files.download', basename($item->file_path)) }}" target="_blank">عرض</a></small>
              </div>
            @endif
          </div>
