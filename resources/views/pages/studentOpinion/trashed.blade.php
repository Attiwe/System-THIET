...
@section('title', ' اراء الطلاب المحذوفه')
...
@section('page-header')
  ...
  <div class="breadcrumb-header justify-content-between">
    <div class="left-content">
      <div>
        <h2 class="main-content-title tx-24 mg-b-1 mg-b-lg-1"> 📋 اراء الطلاب المحذوفه </h2>
        <p class="mg-b-0"> يعرض جدول اراء الطلاب المحذوفه </p>
      </div>
    </div>
  ...
  <div class="card card-body">
    ...
    <div class="d-flex justify-content-between align-items-center">
      <h2 class="text-primary mb-3">📋 اراء الطلاب المحذوفه </h2>
      <a href="{{ route('studentOpinions.index') }}" class="btn btn-outline-primary">
    ...
    <table id="example1"
      ...
      <thead class="px-3 py-2  text-white">
        <tr>
          ...
          <th scope="col" class="h5"><i class="bi bi-chat"></i> التفاصيل</th>
          ...
          <tr>
            ...
            <td class="text-center">
              <div class="img-container position-relative">
                <img src="{{ asset('image/studentOpinions/' . $item->image) }}" alt="{{ $item->name }}" class="img-fluid rounded-circle" style="width: 40px; height: 40px; object-fit: cover;">
              </div>
            </td>
            <td>
              @if($item->is_active)
            ...
            <td class="fw-bold">{{ \Illuminate\Support\Str::limit($item->details, 50, ' ...') ?? 'لا يوجد ملاحظات' }}</td>
            <td>
              <!-- restore -->
              <form action="{{ route('studentOpinions.restore', $item->id) }}" method="POST" class="d-inline">
                @csrf
                @method('HEAD')
              ...
              <!-- delete -->
              <form action="{{ route('studentOpinions.forceDelete', $item->id) }}" method="POST" class="d-inline">
                @csrf
                @method('delete')
