  ...
  <div class="modal-dialog modal-dialog-centered modal-xl">
    <div class="modal-content shadow-lg border-0 rounded-4 overflow-hidden">
        ...
        <div class="position-relative z-1 d-flex align-items-center w-100">
          <div class="flex-grow-1">
            <h4 class="modal-title fw-bold mb-1" id="staticBackdropLabel">
              <i class="bi bi-journal-edit me-2"></i> تعديل التربية العسكرية
            </h4>
            <p class="mb-0 opacity-75 small">قم بتعديل محتوى التربية العسكرية</p>
          </div>
      ...
      <div class="modal-body p-4">
        <form action="{{ route('military-education.update', $item->id) }}" method="POST" id="editForm{{ $item->id }}" class="needs-validation" novalidate>
          @csrf
          @method('PUT')
            ...
            <div class="col-12">
              ...
              <div class="input-group input-group-lg">
                ...
                <textarea class="form-control border-start-0 shadow-sm" name="description" required 
                  ...
                  style="border-radius: 0 0.5rem 0.5rem 0; resize: vertical; min-height: 200px;">{{ $item->description }}</textarea>
              </div>
              <div class="invalid-feedback">يرجى إدخال وصف التربية العسكرية</div>
              <div class="form-text">
                <i class="bi bi-info-circle me-1"></i>
                الحد الأدنى: 10 أحرف | الحد الأقصى: 5000 حرف
                <span class="text-muted ms-2" id="charCount{{ $item->id }}">{{ strlen($item->description) }} / 5000</span>
              </div>
            </div>
          </div>
        </form>
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    if (form) {
      ...
      const charCount = document.getElementById('charCount{{ $item->id }}');
