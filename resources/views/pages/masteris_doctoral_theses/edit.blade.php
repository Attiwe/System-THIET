...
@section('title', 'تعديل رسالة ماجستير/دكتوراه')
...
@section('page-header')
  ...
  <div class="breadcrumb-header justify-content-between">
    <div class="left-content">
      <div>
        <h2 class="main-content-title tx-24 mg-b-1 mg-b-lg-1"> ✏️📚 تعديل رسالة ماجستير/دكتوراه </h2>
        <p class="mg-b-0"> تعديل بيانات الرسالة/الأطروحة </p>
      </div>
    </div>
        ...
        <div class="row align-items-center">
          <div class="col-md-8">
            <div class="d-flex align-items-center">
              ...
              <div>
                <h1 class="mb-2 fw-bold">تعديل رسالة ماجستير/دكتوراه</h1>
                <p class="mb-0 opacity-75">تعديل بيانات الرسالة/الأطروحة: {{ $thesis->title_thesis }}</p>
              </div>
            </div>
          </div>
          <div class="col-md-4">
    ...
    <div class="row justify-content-center">
      <div class="col-lg-10">
        <div class="edit-card">
          ...
          <form action="{{ route('masterisDoctoralTheses.update', $thesis->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="row g-3">
              <div class="col-md-6">
                ...
                <input type="text" class="form-control" id="doctor_name" name="doctor_name" 
                  value="{{ old('doctor_name', $thesis->doctor_name) }}" placeholder="أدخل اسم الدكتور" required>
              </div>
              ...
              <div class="col-md-6">
                ...
                <select name="type" id="type" class="form-select" required>
                  <option value="" {{ old('type', $thesis->type) == '' ? 'selected' : '' }}>اختر النوع</option>
                  <option value="master" {{ old('type', $thesis->type) == 'master' ? 'selected' : '' }}>الماجستير</option>
                  <option value="doctoral" {{ old('type', $thesis->type) == 'doctoral' ? 'selected' : '' }}>الدكتوراه</option>
                  <option value="other" {{ old('type', $thesis->type) == 'other' ? 'selected' : '' }}>غير ذلك</option>
                </select>
              </div>
            </div>
            ...
            <div class="row g-3">
              <div class="col-md-6">
                ...
                <select name="department_id" id="department_id" class="form-select" required>
                  <option value="" {{ old('department_id', $thesis->department_id) == '' ? 'selected' : '' }}>اختر القسم</option>
                  @foreach ($departments as $department)
                    <option value="{{ $department->id }}" {{ old('department_id', $thesis->department_id) == $department->id ? 'selected' : '' }}>
                      {{ $department->name }}
                    </option>
                  @endforeach
                </select>
              ...
              <div class="col-md-6">
                ...
                <div class="form-text">
                  @if($thesis->thesis_pdf)
                    الملف الحالي: <a href="{{ route('masterisDoctoralTheses.showPdf', $thesis->id) }}" target="_blank" class="text-primary">عرض الملف</a>
                  @else
            ...
            <div class="row g-3">
              <div class="col-12">
                ...
                <input type="text" class="form-control" id="title_thesis" name="title_thesis" 
                  value="{{ old('title_thesis', $thesis->title_thesis) }}" placeholder="أدخل عنوان الرسالة/الأطروحة" required>
              </div>
            </div>
            <br>
            ...
            <div class="row g-3">
              <div class="col-12">
                ...
                <textarea class="form-control" id="description" name="description" rows="6" 
                  placeholder="أدخل وصف مفصل للرسالة/الأطروحة" required>{{ old('description', $thesis->description) }}</textarea>
              </div>
            </div>
            ...
            <div class="d-flex justify-content-center gap-3 mt-4">
              ...
              <a href="{{ route('masterisDoctoralTheses.index') }}" class="btn btn-outline-secondary btn-lg px-5">
