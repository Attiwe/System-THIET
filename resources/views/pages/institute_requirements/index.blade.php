...
@section('page-header')
  ...
  <div class="breadcrumb-header justify-content-between">
    <div class="left-content">
      <div>
        <h2 class="main-content-title tx-24 mg-b-1 mg-b-lg-1"> 📋 جدول متطلبات المعهد </h2>
        <p class="mg-b-0"> يعرض جدول متطلبات المعهد والملفات المرتبطة بها </p>
      </div>
    </div>
  ...
  <div class="card card-body">
    <div class="table-responsive mt-3">
      <h2 class="text-primary mb-3">📋 جدول متطلبات المعهد</h2>

      <div class="d-flex justify-content-center">
        <button type="button" class="btn btn-outline-primary" data-bs-toggle="modal" data-bs-target="#staticBackdrop">
          <i class="bi bi-plus-circle"></i> <strong class="h5 font-weight-bold"> إضافة متطلبات المعهد</strong>
        </button>
      </div>

      <!-- Model institute_requirements -->
      @include('pages.institute_requirements._create')
      ...
      <table id="example1"
        ...
        <tbody>
          @foreach($instituteRequirements as $requirement)
            <tr>
              <td class="fw-bold text-dark">{{ $loop->iteration }}</td>
              <td class="fw-bold text-dark">{{ $requirement->department->name ?? 'غير محدد' }}</td>
              <td>
                @if($requirement->file_path)
                  <button type="button" class="btn btn-outline-danger btn-sm" 
                          onclick="viewFile('{{ asset('storage/institute_requirements/' . $requirement->file_path) }}', 'متطلبات المعهد - {{ $requirement->department->name ?? 'غير محدد' }}')">
              ...
              <td class="fw-bold text-info">{{ $requirement->created_at->format('Y-m-d') }}</td>
              <td>
                <div class="dropdown">
                  <button class="btn btn-outline-primary btn-sm dropdown-toggle" type="button" id="dropdownMenuButton{{ $requirement->id }}"
                  ...
                  <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton{{ $requirement->id }}">
                    ...
                    <li>
                      <form action="{{ route('institute_requirements.destroy', $requirement->id) }}" method="POST" class="d-inline">
                        @csrf
                        @method('delete')
            ...
            <!-- model edit -->
            @include('pages.institute_requirements._edit', compact('requirement'))
          @endforeach
        </tbody>
      </table>
    </div>
...
@section('js')
  ...
  @include('pages.institute_requirements._delete')
@endsection
